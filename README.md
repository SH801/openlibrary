...
## Overview
The `openlibrary` library provides a bootstrapper library that loads a particulary library referenced in a `yml` URL and then runs the library on that `yml` file. It can be used to create separate custom Python libraries that operate on parameters stored in a `yml` file while ensuring the necessary external library is installed at runtime.
